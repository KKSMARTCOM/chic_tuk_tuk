...
@section('content')
    ...
    <div class="bg-white rounded-lg shadow-md mb-8">
        <div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
            <div>
                <h1 class="text-lg md:text-2xl font-bold text-gray-800">Modifier le Paiement</h1>
                <p class="text-sm md:text-base text-gray-600">Modifier le paiement d'un Agent</p>
            </div>
        ...
        <form action="{{ route('admin.payments.update', $payment) }}" method="POST">
                ...
                <select name="driver_id" required
                    ...
                    @foreach ($drivers as $driver)
                        <option value="{{ $driver->id }}"
                            {{ (old('driver_id') ?? $payment->driver_id) == $driver->id ? 'selected' : '' }}>
            ...
            <div class="mb-6">
                ...
                <select name="payment_type" required
                    ...
                    <option value="commission"
                        {{ (old('payment_type') ?? $payment->payment_type) == 'commission' ? 'selected' : '' }}>Commission
                    </option>
                    <option value="contract"
                        {{ (old('payment_type') ?? $payment->payment_type) == 'contract' ? 'selected' : '' }}>Contractuel
                    </option>
                </select>
                @error('payment_type')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                ...
                <select name="driver_contract_id"
                    ...
                    @foreach ($driverContracts as $contract)
                        <option value="{{ $contract->id }}"
                            {{ (old('driver_contract_id') ?? $payment->driver_contract_id) == $contract->id ? 'selected' : '' }}>
            ...
            <div class="mb-6">
                ...
                <select name="vehicle_contract_id"
                    ...
                    @foreach ($vehicleContracts as $contract)
                        <option value="{{ $contract->id }}"
                            {{ (old('vehicle_contract_id') ?? $payment->vehicle_contract_id) == $contract->id ? 'selected' : '' }}>
            ...
            <div class="mb-6">
                ...
                <select name="payment_method" required
                    ...
                    <option value="cash"
                        {{ (old('payment_method') ?? $payment->payment_method) == 'cash' ? 'selected' : '' }}>Espèces
                    </option>
                    <option value="bank_transfer"
                        {{ (old('payment_method') ?? $payment->payment_method) == 'bank_transfer' ? 'selected' : '' }}>
                        Virement Bancaire</option>
                    <option value="check"
                        {{ (old('payment_method') ?? $payment->payment_method) == 'check' ? 'selected' : '' }}>Chèque
                    </option>
                    <option value="mobile_money"
                        {{ (old('payment_method') ?? $payment->payment_method) == 'mobile_money' ? 'selected' : '' }}>
                        Mobile Money</option>
                    <option value="other"
                        {{ (old('payment_method') ?? $payment->payment_method) == 'other' ? 'selected' : '' }}>Autre
                    </option>
                </select>
            ...
            <div class="mb-6">
                ...
                <textarea name="notes" rows="4" placeholder="Notes supplémentaires..."
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('notes') border-red-500 @enderror">{{ old('notes', $payment->notes) }}</textarea>
                @error('notes')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
