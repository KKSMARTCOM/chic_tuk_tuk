...
@section('content')
    ...
    @if ($requests->isEmpty())
        <div class="bg-green-50 border border-green-200 rounded-lg p-6 text-center">
            ...
            <p class="text-green-600 text-sm mt-2">Toutes les demandes de pause ont été traités.</p>
        </div>
    @else
        <div class="space-y-4">
            @foreach ($requests as $request)
                <div class="bg-white rounded-lg shadow-md p-6 border-l-4 border-yellow-500">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6 p-4 bg-blue-50 rounded-lg">
                        <div>
                            <p class="text-xs text-blue-600 font-semibold uppercase">Jours disponibles</p>
                            <p class="text-2xl font-bold text-blue-900">
                                {{ $request->driver->getAvailableLeaveDays() }}
                            </p>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <p
                                class="text-xl font-bold {{ $request->driver->getAvailableLeaveDays() >= count($request->dates) ? 'text-green-600' : 'text-red-600' }}">
                                {{ $request->driver->getAvailableLeaveDays() >= count($request->dates) ? '✓ Oui' : '✗ Non' }}
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="flex gap-3">
                        @if ($request->driver->getAvailableLeaveDays() >= count($request->dates))
                            <button type="button"
