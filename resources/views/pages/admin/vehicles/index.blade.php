...
@section('content')
                            ...
                            <tr class="hover:bg-gray-50 transition">
                                ...
                                <td class="px-4 py-4 whitespace-nowrap">
                                    <div class="flex items-center gap-2">
                                        <a title="Voir plus" href="{{ route('admin.vehicles.show', $vehicle) }}"
                                            class="text-blue-600 hover:text-blue-800" title="Voir">
                                            <i class="fas fa-eye"></i>
                                        </a>

                                        <button title="Modifier" onclick="openEditModal({{ $vehicle->toJson() }})"
                                        ...
                                        @if ($vehicle->activePause)
                                            <form action="{{ route('admin.vehicles.end-pause', $vehicle->activePause) }}"
                                                ...
                                                <button type="submit" title="Terminer la pause"
                                        ...
                                        <button title="Supprimer"
                                            onclick="openDeleteModal('{{ $vehicle->id }}', '{{ $vehicle->vehicle_number }}')"
    ...
    {{-- ===== MODAL SUPPRESSION ===== --}}
    @include('inc.modals.vehicles.delete')
    ...
    @push('scripts')
        <script>
            ...
            // ===== SUPPRESSION =====
            function openDeleteModal(vehicleId, vehicleNumber) {
                document.getElementById('deleteMessage').textContent =
                    `Êtes-vous sûr de vouloir le véhicule ${vehicleNumber} ? Cette action est irréversible.`;
