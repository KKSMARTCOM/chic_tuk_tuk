...
@section('content')
    ...
    <div class="bg-white rounded-lg shadow-md mb-8">
        <div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-800">Modifier le Circuit Touristique</h1>
                <p class="text-gray-600">{{ $circuit->name }}</p>
            </div>
    ...
    <form action="{{ route('admin.circuits.update', $circuit) }}" method="POST" enctype="multipart/form-data"
        ...
        <div class="px-6 py-6 space-y-6">
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $circuit->name) }}"
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4"
                    ...
                    placeholder="Décrivez le circuit en détail...">{{ old('description', $circuit->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label for="price" class="block text-sm font-medium text-gray-700 mb-2">Prix (FCFA) <span
                            class="text-red-600">*</span></label>
                    <div class="relative">
                        <input type="number" name="price" id="price" value="{{ old('price', $circuit->price) }}"
                            step="0.01" min="0"
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="number" name="duration" id="duration"
                            value="{{ old('duration', $circuit->duration) }}" min="1"
            ...
            <div class="border-t border-gray-200 pt-6">
                ...
                <div id="locationsContainer" class="space-y-2 mb-4">
                    @php
                        $locations = old('locations', $circuit->locations ?? []);
                    @endphp
            ...
            <div class="border-t border-gray-200 pt-6">
                    ...
                    <div class="mb-6">
                        ...
                        <div class="relative">
                            <img src="{{ asset('storage/' . $circuit->image) }}" alt="{{ $circuit->name }}"
                                class="h-48 rounded-md border border-gray-300 object-cover">
                            <button type="button" onclick="removeCurrentImage()"
                                class="absolute top-2 right-2 bg-red-500 text-white p-2 rounded-md hover:bg-red-600 transition">
                                <i class="fas fa-trash"></i>
                            </button>
                        </div>
                ...
                <div>
                    <label for="image" class="block text-sm font-medium text-gray-700 mb-2">Changer l'image
                        (optionnel)</label>
                ...
                <div id="imagePreview" class="mt-4 hidden">
                    <p class="text-sm font-medium text-gray-700 mb-2">Nouvel aperçu:</p>
                    ...
                    <button type="button" onclick="clearImage()"
                        ...
                        <i class="fas fa-trash mr-1"></i>Supprimer la nouvelle image
                    </button>
                </div>
            </div>
    ...
    <div class="bg-red-50 border border-red-200 rounded-lg shadow-md mt-8 overflow-hidden">
        ...
        <div class="px-6 py-6">
            <p class="text-red-700 mb-4">Une fois supprimé, ce circuit ne pourra pas être récupéré. Cela supprimera
                également toutes les réservations associées.</p>
            <form action="{{ route('admin.circuits.destroy', $circuit) }}" method="POST" class="inline-block"
                onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer ce circuit définitivement ?');">
                @csrf
                @method('DELETE')
                <button type="submit"
                    class="bg-red-600 text-white px-6 py-2 rounded-lg hover:bg-red-700 transition font-medium">
                    <i class="fas fa-trash mr-2"></i>Supprimer ce circuit
                </button>
            </form>
        </div>
    </div>
    ...
    @push('scripts')
        <script>
            ...
            function removeCurrentImage() {
                document.getElementById('keepImage').value = '0';
                document.querySelector('[id*="currentImage"]')?.remove?.();
            }
